...
public:
  ...
  // Frees this node's cached text-rendering resource (if any) and
...
inline PangoLayout *makeLayout(const std::string &text, const TextStyle &style,
  ...
  if (style.maxLines > 0) {
    ...
    pango_layout_set_ellipsize(style.overflow == TextOverflow::Ellipsis
                                   ? PANGO_ELLIPSIZE_END
                                   : PANGO_ELLIPSIZE_NONE);
  } else if (style.overflow == TextOverflow::Ellipsis) {
    pango_layout_set_ellipsize(PANGO_ELLIPSIZE_END);
  }
  switch (style.align) {
  case TextAlign::Center:
...
// Windows-only member/method block.
  ...
  //
  // Known caveat: this uploads with GL_BGRA_EXT, matching cairo's native
  // ARGB32 byte order on the little-endian platforms this file targets.
  // On a GLES implementation lacking EXT_texture_format_BGRA8888, swap
  // the B/R channels in `data` before upload (or upload as GL_RGBA and
  // swap in the fragment shader) instead.
  static void ensureTextTexture(const View &v) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, innerW, innerH, 0, GL_BGRA_EXT,
