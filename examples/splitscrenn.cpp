...
int main() {
  LiteUI ui("Resizable Split Screen", 900, 600);
  ...
  View left;
  left.style.width =
      std::function<Size()>([splitX] { return Size::pixel(*splitX); });
  ...
  // drift out of sync.
  auto updateSplitFromLocalX = [splitX, dividerWidth,
                                minPaneWidth](float localX) {
